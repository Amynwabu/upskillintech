...
const FEATURED = [
  {
    icon: <BookOpen size={28} style={{ color: "#439288" }} />,
    badge: "Free Guide",
...
const CASE_STUDIES = [
  { icon: <Briefcase size={22} style={{ color: "#439288" }} />, title: "Professional Use Cases", desc: "Examples of individuals integrating AI into their daily work — from consultants to educators.", href: "/resources/case-studies" },
...
export default function Resources() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      >
        ...
        <div className="container relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center py-20 lg:py-28">
            <div>
              <span className="inline-block text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-4" style={{ background: "rgba(67,146,136,0.10)", color: "#F3F4F6", border: "1px solid rgba(255,255,255,0.35)" }}>Resource Hub</span>
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/resources/ai-guides" className="inline-flex items-center gap-2 font-semibold px-6 py-3 rounded-lg transition-all" style={{ background: "#151B23", color: "#439288", fontFamily: "'Sora', sans-serif" }}>
            ...
            <div className="hidden lg:grid grid-cols-2 gap-4 max-w-sm w-full ml-auto">
              {[
                { icon: <BookOpen size={28} style={{ color: "#439288" }} />, label: "AI Guides", count: "12+" },
                ...
                { icon: <Zap size={28} style={{ color: "#439288" }} />, label: "Workflows", count: "15+" },
              ].map((item) => (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {BLOG_CATEGORIES.map((cat) => (
              <Link key={cat.title} href={cat.href} className="group rounded-xl p-6 flex items-start gap-4 transition-all hover:-translate-y-1" style={{ background: "#151B23", border: "1px solid #1F2937", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
                <div className="rounded-xl p-3 shrink-0" style={{ background: "#10231C", color: "#439288" }}>{cat.icon}</div>
                <div>
                  ...
                  <span className="inline-flex items-center gap-1 text-xs font-semibold mt-3" style={{ color: "#439288" }}>View Articles <ChevronRight size={14} /></span>
                </div>
              </Link>
            ))}
            <Link href="/resources/blog" className="rounded-xl p-6 flex flex-col items-center justify-center text-center transition-all hover:-translate-y-1" style={{ background: "linear-gradient(135deg, #439288, #8B9E1A)", boxShadow: "0 4px 20px rgba(67,146,136,0.25)" }}>
                ...
                {AI_GUIDES.map((g) => (
                  <div key={g.title} className="flex items-start gap-4 rounded-xl p-5" style={{ background: "#151B23", border: "1px solid #1F2937" }}>
                    <div className="rounded-lg p-2 shrink-0" style={{ background: "#10231C" }}>
                      <BookOpen size={20} style={{ color: "#439288" }} />
                    </div>
                    ...
                    <Link href={g.href} className="shrink-0 text-sm font-semibold flex items-center gap-1" style={{ color: "#439288" }}>
                      {g.cta} <ChevronRight size={14} />
                    </Link>
                  </div>
                ))}
              </div>
              <Link href="/resources/ai-guides" className="inline-flex items-center gap-2 mt-8 font-semibold px-6 py-3 rounded-lg" style={{ background: "#439288", color: "#fff", fontFamily: "'Sora', sans-serif" }}>
      ...
      <section className="section-py" style={{ background: "#151B23" }}>
        <div className="container">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-10">
            {WEBINARS.map((w) => (
              <div key={w.title} className="rounded-xl overflow-hidden" style={{ background: "#151B23", border: "1px solid #1F2937", boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
                <div className="h-2" style={{ background: w.type === "Upcoming" ? "#E6B800" : "#439288" }} />
                <div className="p-6">
                  ...
                  <Link href={w.href} className="inline-flex items-center gap-2 text-sm font-semibold" style={{ color: w.type === "Upcoming" ? "#8B6914" : "#439288" }}>
          ...
          <div className="text-center">
            <Link href="/resources/webinars" className="inline-flex items-center gap-2 font-semibold px-6 py-3 rounded-lg" style={{ background: "#439288", color: "#fff", fontFamily: "'Sora', sans-serif" }}>
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {WORKFLOWS.map((wf) => (
              <Link key={wf.title} href={wf.href} className="group rounded-xl p-6 transition-all hover:-translate-y-1" style={{ background: "#151B23", border: "1px solid #1F2937" }}>
                ...
                <span className="inline-flex items-center gap-1 text-sm font-semibold mt-4" style={{ color: "#439288" }}>Explore Workflow <ChevronRight size={14} /></span>
              </Link>
            ))}
          </div>
          <div className="text-center mt-10">
            <Link href="/resources/workflows" className="inline-flex items-center gap-2 font-semibold px-6 py-3 rounded-lg" style={{ background: "#439288", color: "#fff", fontFamily: "'Sora', sans-serif" }}>
        ...
        <div className="container max-w-2xl mx-auto text-center">
          ...
          <form onSubmit={handleSearch} className="flex gap-3 mb-6">
            <input
              ...
              style={{ border: "2px solid #439288", fontFamily: "'DM Sans', sans-serif" }}
            />
            <button type="submit" className="flex items-center gap-2 font-semibold px-5 py-3 rounded-lg" style={{ background: "#439288", color: "#fff" }}>
              <Search size={18} />
            </button>
          </form>
          <div className="flex flex-wrap gap-2 justify-center">
            ...
            {POPULAR_TOPICS.map(t => (
              <button key={t} onClick={() => { setSearchQuery(t); toast.success(`Searching "${t}"…`); }} className="text-sm px-3 py-1 rounded-full transition-all hover:bg-green-100" style={{ background: "#151B23", border: "1px solid #439288", color: "#439288" }}>
                {t}
              </button>
            ))}
          </div>
      ...
      <section className="section-py" style={{ background: "#1C1C1C" }}>
        <div className="container">
          <div className="max-w-3xl mx-auto rounded-2xl p-10 text-center" style={{ background: "linear-gradient(135deg, #439288 0%, #8B9E1A 60%, #E6B800 100%)" }}>
      ...
      <section className="section-py" style={{ background: "#151B23" }}>
        <div className="container text-center max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <Link href="/#programs" className="inline-flex items-center gap-2 font-semibold px-7 py-3 rounded-lg" style={{ background: "#439288", color: "#fff", fontFamily: "'Sora', sans-serif" }}>
              <ArrowRight size={18} /> Explore Programs
            </Link>
            <Link href="/#community" className="inline-flex items-center gap-2 font-semibold px-7 py-3 rounded-lg border-2" style={{ borderColor: "#439288", color: "#439288", fontFamily: "'Sora', sans-serif" }}>
